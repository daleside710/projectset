...
class PageController extends Controller
{
    ...
    public function spin(Request $request)
    {
        ...
        if ($request->has('coupon_code')) {
            ...
            $box->price = intval($box->price * 100); //Because in wallet amount is 00
            auth()->user()->pay($box);
...
//            dd('its free box');
            ...
            if ($diffInMinutes < 1440 && (isset($boxIsTimeCompleted->is_notified) && $boxIsTimeCompleted->is_notified != 1)) {
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'Du kan ikke åpne boksen enda. Vennligst vent til nedtelling har fullført.'
                ]);
    ...
    public function sellBack(Request $request)
    {
        ...
        $product->product->sell_back_price = $product->product->sell_back_price * 100; //Because in wallet amount is 00
    ...
    public function home(Request $request)
    {
        ...
        Session::forget('sort_search');

        $q = Box::query();
        $q->where('is_published', 1)->orderBy('rank', 'Asc');

        if(isset($request->name)){
            Session::put('search_name',$request->name);
            $q->where('name','LIKE','%'.$request->name.'%');
        }

        if(isset($request->sort_search) && $request->sort_search == 'low_price'){
            Session::put('sort_search','low_price');
            $q->orderBy('price','Asc');
        }

        if(isset($request->sort_search) && $request->sort_search == 'high_price'){
            Session::put('sort_search','high_price');
    ...
    {
        $privacy = DB::table('privacy')->where('id', 1)->first();
    ...
    {
        $safety = DB::table('safety')->where('id', 1)->first();

        return view('frontend.pages.safety', compact('safety'));
    }
    ...
    public function createOrder(Request $request)
    {
        try {
            ...
            if ($request->delivery_fees > 0) {
                $request->delivery_fees = $request->delivery_fees*100; 
            ...
            return redirect()->route('home')->with('success', 'Din bestilling er mottatt. Du vil se bestillingsstatus på Min konto > Bestillinger.');
    ...
    public function applyCouponCode(Request $request)
    {
        ...
        try {
            ...
            $coupon = CouponCode::whereCode($request->coupon_code)
    ...
    public function openBox($boxId)
    {
        ...
        return redirect()->route('redeem',$boxId)->with('success', 'Boksen har blitt låst opp og nedtelling har startet.');

    }

    public function getCurrentTime()
    {
        return response()->json(['datetime' => Carbon::now()->format('Y-m-d H:i:s')]);
    }
