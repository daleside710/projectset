                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td><input value="{{ $box->price / 100 }}" type="text" class="form-control" name="price" required></td>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody id="addedProducts">
                                    ...
                                    <tr>
                                        <td>{{ $box_product->product->id }}</td>
                                        <td>
                                            <figure class="avatar avatar-sm border-0 mr-2">
                                                <img class="rounded-circle" src="{{ Storage::disk('s3')->url($box_product->product->image_path) }}">
                                            </figure>
                                            <div class="d-inline-block">
                                                <h6 class="font-weight-light">{{ $box_product->product->name }}</h6>
                                            </div>
                                        </td>
                                        <td>{{ number_format($box_product->product->sell_back_price / 100, 0, '.', ' ') }},-</td>
                                        <td>
                                            <input style="height: calc(1.5em + .75rem + 2px); width: 135px"
                                            value="{{ $box_product->wining_chance }}"
                                            name="products[{{ $box_product->product->id }}][wining_chance]" class="form-control" type="text" required>
                                        </td>
                                        <td class="text-center text-uppercase">
                                            @if(Auth::user()->type === 3)
                                                @if($box_product->product->disabled === 0 )
                                                    <button type="button" class="btn btn-sm btn-warning mr-2 disableBTN" data-product-id="{{$box_product->product->id}}">Deaktiver</button>
                                                @else
                                                    <button type="button" class="btn btn-sm btn-success mr-2 enableBTN" data-product-id="{{$box_product->product->id}}">Aktiver</button>
                                                @endif
                                            @endif
