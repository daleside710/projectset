...
<div class="container" style="margin-top: -50px">
  ...
  <div class="team-roster-nav js-team-roster-nav">
    ...
    <div class="team-roster-nav__meta float_right">
      ...
      <h5 class="team-roster-nav__nickname mb-2">{{ number_format($box->price, 0, ',', ' ') }},- NOK</h5>
      @if(auth()->user())
          @if(auth()->user()->type === 1 or auth()->user()->type === 3)
            <div style="cursor: not-allowed">
                <a href="{{ route('redeem', [$box->id]) }}" class="btn btn-primary-turquoise btn-sm {{ (auth()->user()->balance/100 < $box->price) ? 'disabled' : '' }}">
                  Åpne for kr. {{ number_format($box->price, 0, ',', ' ') }},-
                </a>
            </div>

            @if((auth()->user()->balance)/100 < $box->price)
            ...
            <header class="product__header">
              <div class="product__header-inner">
                ...
                <div class="product__price">
                    {{ number_format($boxProduct->sell_back_price, 0, ',', ' ') }},- NOK
                </div>
              </div>
            </header>
          </div>
